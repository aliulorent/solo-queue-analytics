...
export const useRankedStore = defineStore('ranked', ()=> {
    ...
    const fetchRankedStats = async ()=>{
        ...
        statusCode.value = 0;
        try{
            const res = await fetch(`https://solo-queue-analytics.onrender.com/getRankedStats/${player.puuid}`, {method: 'GET', mode:'cors'});
    ...
    const updateRankedStats = async ()=>{
        ...
        statusCode.value = 0;
        try{
            const res = await fetch(`https://solo-queue-analytics.onrender.com/updateRankedStats/${player.region}/${player.summoner_id}/${player.puuid}`, {method: 'PUT', mode:'cors'});
            if(res.status && res.status===200){
                //Successfully recieved a response
                const data = await res.json();
                solo_tier.value = data.solo_tier;
